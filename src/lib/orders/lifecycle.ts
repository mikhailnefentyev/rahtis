...
async function explain(
   ...
   * молчать о ней нельзя. Так и вышло с отказом от рейса: функция падала
   * на приведении типа (42804), человек видел «попробуйте ещё раз», и в
   * логах не оставалось ничего. Общая фраза остаётся — показывать код
   * Postgres перевозчику незачем, — но сама ошибка теперь доезжает до
   * журнала площадки.
   */
  console.error('lifecycle: неизвестный отказ базы:', code, message);
