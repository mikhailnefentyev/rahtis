...
/**
 * Есть ли у точки координаты, и настоящие ли они.
 *
 * Проверка одна на публикацию и на правку, потому что правило одно:
 * платформа принимает только адрес, выбранный из подсказки. Смысл этого
 * не в аккуратности карты, а в том, что километраж считается по
 * координатам, а от километража считается ставка. Адрес без координат
 * означает километраж, вписанный руками, — а его после публикации уже
 * не исправить: distance_km неизменен, потому что о цене договорились.
 *
 * Проверяются не только пустые строки: скрытые поля координат едут
 * формой и приходят от браузера, то есть от чего угодно. Нуль-остров
 * (0, 0) в Гвинейском заливе — самый частый исход разбора мусора, и
 * маршрут до него считается совершенно исправно.
 */
export function hasCoordinates(stop: { lat?: string; lon?: string }): boolean {
  const lat = Number.parseFloat(stop.lat ?? '');
  const lon = Number.parseFloat(stop.lon ?? '');
