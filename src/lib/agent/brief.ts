...
export function agentTools(audience: Audience): ToolSpec[] {
  ...
  const tools: ToolSpec[] = [
    ...
    },
    {
      name: 'place_guide',
      description:
        'Площадка из справочника: адрес ворот и координаты. Часов работы и порядка въезда в справочнике НЕТ — не додумывай их, а передай вопрос оператору инструментом ask_operator.',
      input_schema: {
        type: 'object',
        properties: {
          query: {
  ...
  /*
   * Вопрос человеку. Единственный инструмент, который пишет.
   *
   * До него помощник кончался советом «напишите оператору» — и это был
   * тупик: кто такой оператор и где ему пишут, собеседник не знал, а
   * помощник для того и заведён, чтобы спрашивать было куда. Совет
   * обратиться к человеку без способа это сделать не лучше молчания.
   *
   * Оператору инструмент не даётся: он и есть тот, кому пишут.
   */
  if (!admin) {
    tools.push({
      name: 'ask_operator',
      description:
        'Передать вопрос оператору платформы (Aivomaa Oy) в очередь поддержки. '
        + 'Зови, когда ответа нет в данных: часы работы площадки, порядок въезда, '
        + 'спорная сумма, повреждение груза, всё, чего нет у остальных инструментов. '
        + 'Вопрос пересказывай полностью — оператор не видит этой переписки. '
        + 'Ответ оператора приходит уведомлением в кабинет и письмом, а не в этот чат.',
      input_schema: {
        type: 'object',
        properties: {
          question: {
            type: 'string',
            description: 'Вопрос целиком, своими словами, с номером заказа, если он известен',
          },
          subject: { type: 'string', description: 'Тема одной строкой' },
        },
        required: ['question'],
      },
    });
...
export function agentSystemPrompt(input: {
  ...
  if (admin) {
    lines.push(
      ...
      'Выручка оператора с 23.09.2026, две ветки. Перевозчик-подписчик (companies.partnership = SUBSCRIBER) возит своих клиентов: 29,90 € + ALV в месяц за машину, закрывшую рейс (carrier_subscription_fees, удержания — carrier_fee_deductions); в деньгах и документах его собственных рейсов мы не участвуем. Перевозчик-субподрядчик (SUBCONTRACTOR) платит 3 % с цены рейса (orders.commission_bps), месячного сбора у него нет. Сторона договора записана в самом рейсе: orders.contract_party = RAHTIS — наш договор и 3 % с перевозчика, CARRIER — рейс своего клиента без процента и без нашего счёта. Заказ со стола всегда идёт как подряд, поэтому там 3 % с заказчика (orders.shipper_fee_bps) и 3 % с перевозчика; прямой заказ постоянной машине — 0 % с заказчика. Оборот, выручка и выплаты — разные числа, не путай их.',
  ...
  /*
   * Кто такой оператор и как до него дойти.
   *
   * Прежде здесь стояло «предложи написать оператору» — совет, который
   * некуда применить: ни имени, ни адреса, ни кнопки. Теперь у помощника
   * есть инструмент, и правило говорит не «предложи», а «сделай»:
   * спрашивать человека за собеседника и есть его работа.
   */
  if (admin) {
    lines.push(
      'Инструмента не хватило или данных нет — так и скажи. Оператор здесь ты сам, передавать вопрос некому.',
    );
  } else {
    lines.push(
      'Оператор платформы — Aivomaa Oy, это мы: не заказчик и не перевозчик, а те, кто ведёт платформу.',
      'Данных нет или вопрос не к данным — не отсылай собеседника никуда, а зови ask_operator сам.',
      'Перескажи вопрос целиком: оператор видит только то, что ты написал, а не эту переписку.',
      /*
       * Куда придёт ответ — по правде. Оператор отвечает из админки
       * уведомлением в кабинет и письмом на почту компании; в этот
       * тред его ответ не попадает, и обещать чат значило бы
       * оставить человека ждать в пустом окне.
       */
      'Позвал — скажи об этом прямо: вопрос передан оператору, ответ придёт уведомлением в кабинет '
        + 'и письмом на почту компании. Не обещай ответ в этом чате: сюда он не приходит.',
    );
  }
