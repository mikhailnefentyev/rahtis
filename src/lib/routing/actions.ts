...
export async function suggestAddressAction(
  ...
  /*
   * Подсказку спрашивает не только заказчик.
   *
   * Пока адрес набирали лишь в форме заказа, проверка на SHIPPER была
   * верной. С тех пор из подсказки выбирается ещё и база машины в
   * карточке перевозчика — и там она молча возвращала отказ: поле
   * работало как обычный текст, координат не давало, машина не попадала
   * на карту транспорта. Увидеть это нельзя: список подсказок просто не
   * появляется, ровно как когда геокодер ничего не нашёл.
   *
   * Сторона сделки здесь ни при чём: вызов стоит денег, поэтому он
   * закрыт от анонима, а не от роли. Расчёт маршрута ниже остаётся за
   * заказчиком — там от ответа зависит цена рейса.
   */
