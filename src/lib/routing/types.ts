...
/**
 * Чем отличается расчёт «когда приеду» от расчёта километража.
 *
 * Километраж заказа не должен зависеть от минуты, в которую заказчик
 * нажал кнопку: та же сцепка по тому же маршруту обязана дать те же
 * километры и завтра, и в час пик. Поэтому в обычном расчёте пробок нет,
 * и время в пути там — оценка по пустой дороге.
 *
 * Водителю нужно ровно обратное. Вопрос «успею ли к 14:00» без пробок не
 * имеет смысла: перед Вуосаари в пятницу вечером теряется час, и ответ
 * без него не просто неточен, а вреден — он говорит «успеваешь» там, где
 * человек опоздает.
 *
 * Поэтому пробки не переключатель по умолчанию, а признак вызова: их
 * просит тот, кто спрашивает про время, и не просит тот, кто считает
 * расстояние.
 */
...
export interface RoutingProvider {
  ...
  route(points: LatLon[], profile: TruckProfile, options?: RouteOptions): Promise<RouteResult>;
}
