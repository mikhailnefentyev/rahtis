...
export function SiteHeader() {
  ...
  useEffect(() => {
    ...
    /*
     * Узкий экран получает подложку сразу, как только страницу тронули.
     *
     * Первый экран на телефоне высотой не в экран, а в полтора-два:
     * заголовок, два абзаца, кнопки, приметы, счётчик и карточка рейса
     * идут столбиком. Пока подложки не было, всё это проезжало под
     * прозрачной шапкой и сходилось с маркой в одном месте: замер на
     * 390 показал, что на прокрутке 600 надпись кнопки «Huolitsijalle
     * tai operaattorille» лежит ровно поверх логотипа и обрезается
     * переключателем языка. Читались обе надписи как каша.
     *
     * На широком экране первый экран и есть экран: до метки его конца
     * доезжают одним движением, и лишняя полоса поверх съёмки там
     * ничего не спасает, а кадр портит. Поэтому порог только для узких.
     */
    const narrow = window.matchMedia('(max-width: 1000px)');

    const measure = () => {
      frame = 0;
      const passed = sentinel.getBoundingClientRect().top <= HEAD_HEIGHT;
      setSolid(passed || (narrow.matches && window.scrollY > 24));
    };
