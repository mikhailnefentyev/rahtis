-- ═══════════════════════════════════════════════════════════════════
-- RAHTIS · номер брони переезжает в инструкции к точке
--
-- Отдельная графа под бронь была лишней. Прицеп ищут по номеру — он
-- теперь обязателен у заказа, — а референс нужен не всегда и выглядит
-- каждый раз по-своему: у одного порта это номер брони, у другого номер
-- визита, у третьего пропуск. Заводить под каждый вариант своё поле
-- значит плодить графы, которые в большинстве заказов остаются пустыми.
--
-- Всё это пишется в инструкции к точке — там, где водитель и читает, что
-- сказать на воротах.
--
-- Записанные брони не выбрасываются: они переносятся в те же инструкции,
-- к тексту, который там уже есть. Удалить столбец с данными и объявить
-- это упрощением было бы потерей, замаскированной под уборку.
-- ═══════════════════════════════════════════════════════════════════

update public.order_stops
set note = case
      when nullif(btrim(coalesce(note, '')), '') is null then 'Бронь: ' || booking_ref
      else note || E'\nБронь: ' || booking_ref
    end
where nullif(btrim(coalesce(booking_ref, '')), '') is not null;

alter table public.order_stops
  drop constraint if exists order_stops_booking_ref_length,
